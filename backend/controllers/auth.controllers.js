...
const login = async (req, res, next) => {
    try {
        const existingUser = await Admin.findOne({email: req.body.email}, {password: 1, name: 1, email: 1});
        ...
        //obtencion de token:
        const token = jwt.sign(userData, process.env.TOKEN_SECRET, { expiresIn: "4h" });
        //respuesta:
...
const tokenLogin = async (req, res, next) => {
    try {
        ...
        const existingUser = await Admin.findById(req.userData._id, {name: 1, email: 1});
        if (!existingUser) throw new HttpError("Usuario no encontrado o inexistente.", 404)
        return res.status(200).json({userData: existingUser, message: 'Sesion iniciada correctamente. Bienvenid@!'})
    } catch (err) {
        return next(err)
    }
};
...
const getResetMail = async (req, res, next) => {
    try {
        ...
        if (!existingUser) throw new HttpError("No existe ningun usuario registrado con este email en la base de datos. Por favor, chequea tus entradas.", 404);
...
module.exports = {login, logout, tokenLogin, getResetMail, verifyResetToken, resetPass}